...
func NotFoundHandler() Handler { return HandlerFunc(NotFound) } // 注册
```
...
type Server struct {
    ...
    doneChan          chan struct{} // doneChan 是否 close 代表 server 的关闭状态
    onShutdown        []func()      // 通过 RegisterOnShutdown 注册，在 Shutdown 时调用当中的钩子函数
}

// 此接口由 ResponseWriters 执行去检测连接是否已断开，此机制允许客户端断开后服务端取消一个长耗时耗力的无效 response 工作
...
type conn struct {
    ...
    lastMethod string
    curReq     atomic.Value // 当前 request
    curState   atomic.Value // 当前连接状态
